...
import { FaRegNewspaper, FaHeart, FaWater, FaUsers, FaClock, FaCheckCircle, FaClipboardList } from 'react-icons/fa';
  ...
  return (
    <div className="w-full bg-[#f8f9fa] min-h-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          { title: "Total Publikasi", count: 36, active: "Berita & Pengumuman", action: "+ Tambah Publikasi", icon: <FaRegNewspaper />, color: "text-blue-600" },
          { title: "Antrean E-Sakinah", count: 12, active: "Perlu Verifikasi", action: "Lihat Berkas", icon: <FaHeart />, color: "text-pink-600" },
          { title: "Reservasi KISS", count: 8, active: "Kunjungan Mendatang", action: "Kelola Jadwal", icon: <FaWater />, color: "text-cyan-600" },
          { title: "Pegawai Aktif", count: 18, active: "Struktur Organisasi", action: "Kelola Pegawai", icon: <FaUsers />, color: "text-orange-600" },
        ].map((stat, idx) => (
          <div key={idx} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            <div className="text-[10px] text-gray-500 font-medium mt-4 pt-3 border-t border-gray-50 flex justify-between items-center">
              <span className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-green-500"></div>{stat.active}</span>
              <span className="text-[#107058] cursor-pointer hover:underline font-bold">{stat.action}</span>
            </div>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:col-span-1">
          <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-2">
            ...
            <span className="text-[10px] font-semibold text-[#107058] cursor-pointer hover:underline">Lihat Semua</span>
          </div>
          <div className="space-y-4">
            {[
              ...
              { title: "Jadwal Pelayanan Keliling Administrasi", cat: "Pengumuman" }
            ].map((item, i) => (
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 lg:col-span-2">
          <div className="flex justify-between items-center mb-6 border-b border-gray-100 pb-2">
            <h3 className="font-bold text-gray-800 text-sm">Perlu Verifikasi (E-Sakinah)</h3>
            <span className="text-[10px] font-semibold text-[#107058] cursor-pointer hover:underline">Buka Halaman E-Sakinah</span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100 text-[10px] text-gray-400 uppercase tracking-wider">
                  <th className="pb-3 font-semibold">Pemohon</th>
              ...
              <tbody className="text-xs text-gray-700">
                {[
                  { p1: "Muhammad", p2: "Kurniawati", tgl: "28 Juli 2026", status: "Menunggu" },
                  { p1: "Saifuddin", p2: "Novianti", tgl: "27 Juli 2026", status: "Menunggu" },
                  { p1: "Ricky Harun", p2: "Dini Kurnia", tgl: "25 Juli 2026", status: "Diproses" },
                  { p1: "Saiful Anwar", p2: "Ekky Nanda", tgl: "24 Juli 2026", status: "Diproses" }
                ].map((row, i) => (
                  <tr key={i} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                    <td className="py-3 font-bold">{row.p1}</td>
                    <td className="py-3">{row.p2}</td>
                    <td className="py-3 text-gray-500">{row.tgl}</td>
                    <td className="py-3">
                      <span className={`px-2.5 py-1 rounded-md text-[9px] font-bold ${row.status === 'Menunggu' ? 'bg-orange-50 text-orange-600 border border-orange-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>
                        {row.status}
                      </span>
                    </td>
                    <td className="py-3 text-center">
                      <button className="bg-gray-100 text-gray-600 hover:bg-[#107058] hover:text-white p-1.5 rounded transition-colors"><FaClipboardList /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
